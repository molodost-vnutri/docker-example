...
async def create_user_item(item: schemas.ItemCreate, user_id: int):
    query = items.insert().values(**item.model_dump(), owner_id=user_id)
    item_id = await database.execute(query=query)
    return schemas.Item(**item.model_dump(), id=item_id, owner_id=user_id)